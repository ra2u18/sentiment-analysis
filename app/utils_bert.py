...
def batch_iter(data, batch_size, shuffle=False, bert=None):
    ...
    for i in range(batch_num):
        ...
        if bert:
            examples = data.iloc[indices].sort_values(by='preprocessed_text_bert'+bert+'_length', ascending=False)
            sents = list(examples.preprocessed_text_bert)
        else:
            examples = data.iloc[indices].sort_values(by='preprocessed_text_length', ascending=False)
            sents = [text.split(' ') for text in examples.preprocessed_text_bert]

        targets = list(examples.information_label.values)
